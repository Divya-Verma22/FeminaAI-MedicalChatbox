...
const Navbar = () => {
  ...
  return (
    ...
    >
        ...
        <nav className="hidden md:flex items-center space-x-8">
          {["About", "How It Works", "FAQs"].map((item, idx) => (
            <a
              ...
              className="text-purple-700 hover:text-yellow-400 transition-colors duration-200 font-medium"
            >
              {item}
            </a>
          ))}
          <Button
            ...
            className="bg-purple-200 hover:bg-purple-300 text-purple-900 rounded-full px-5 py-2 transition-colors shadow-md hover:shadow-lg"
          >
            Chat Now
          </Button>
        </nav>
      ...
      {/* Mobile Nav */}
      {isMobileMenuOpen && (
        <nav className="mt-4 pb-4 md:hidden flex flex-col space-y-4 bg-pink-100 rounded-2xl p-6 shadow-md backdrop-blur-sm">
          {["About", "How It Works", "FAQs"].map((item, idx) => (
            <a
              key={idx}
              href={`#${item.toLowerCase().replace(/ /g, "-")}`}
          ...
          <Button
            onClick={() => navigate("/chat")}
            className="bg-purple-200 hover:bg-purple-300 text-purple-900 w-full rounded-full px-4 py-2 transition-colors"
          >
            Chat Now
          </Button>
        </nav>
...
const ChatPage = () => {
  ...
  const sendMessage = async (content: string) => {
    ...
    try {
      const response = await axios.post(`${API_URL}/api/chat`, { message: content, sessionId });
      const botMessage: Message = {
        role: "bot",
        content: response.data.data || "Sorry, I encountered an error.",
        timestamp: Date.now(),
      };
      ...
      setMessages((prev) => [
        ...prev,
        { role: "bot", content: "Sorry, I encountered an error.", timestamp: Date.now() },
      ]);
    } finally {
      setIsTyping(false);
    }
                    ...
                    <ReactMarkdown
                      ...
                      components={{
                        code({ node, inline, className, children, ...props }) {
                          return !inline ? (
                            <SyntaxHighlighter style={vscDarkPlus} PreTag="div" {...props}>
